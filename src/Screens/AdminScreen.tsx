...
import AddMovieModal from '../Components/AddMovieModal';
import EditMovieModal from '../Components/EditMovieModal';
import ProfileModal from '../Components/ProfileModal'; // ✅ Imported ProfileModal
import Toast from 'react-native-toast-message';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { useNavigation } from '@react-navigation/native';
import { MovieAPIResponse, Movie } from '../types/AdminType';
...
const AdminScreen = () => {
  ...
  const [showProfileModal, setShowProfileModal] = useState<boolean>(false); // ✅ Profile Modal state
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => setShowProfileModal(true)} testID="profile-button">
          <Icon name="person" size={26} color="#FF0000" />
        </TouchableOpacity>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          onEndReached={handleLoadMore}
          onEndReachedThreshold={0.5}
          ListFooterComponent={loadingMore && <ActivityIndicator size="small" color="#FF0000" />}
          testID="movies-flatlist"
        />
      )}
      ...
      <ProfileModal visible={showProfileModal} onClose={() => setShowProfileModal(false)} /> {/* ✅ Used ProfileModal */}
    </SafeAreaView>
  );
};
